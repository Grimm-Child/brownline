...
DEBUG = config('DJANGO_DEBUG', 'off') == 'on'
...
SECURE_PROXY_SSL_HEADER = config('HTTP_X_FORWARDED_PROTO', 'https')

# Redirect all requests to HTTPS
SECURE_SSL_REDIRECT = config('DJANGO_SECURE_SSL_REDIRECT', 'off') == 'on'
...
ELASTICSEARCH_ENDPOINT = config('ELASTICSEARCH_ENDPOINT', '')
    ...
    WAGTAILSEARCH_BACKENDS = {
        'default': {
            ...
            'HOSTS': [{
                ...
                'port': int(config('ELASTICSEARCH_PORT', '9200')),
                'use_ssl': config('ELASTICSEARCH_USE_SSL', 'off') == 'on',
                'verify_certs': config('ELASTICSEARCH_VERIFY_CERTS', 'off') == 'on',
            }],
            'OPTIONS': {
